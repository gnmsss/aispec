# rules/dotnet-server/common/observability.md

## 文档目标
1. 定义 C#/.NET 服务端日志、指标、链路追踪、告警的统一约束。
...
1. 必须使用结构化日志（推荐 Serilog + JSON Sink），禁止只打拼接字符串。
2. 每条请求链路必须记录 `RequestId`、`TraceId`，有用户上下文时记录 `UserId`。
3. 日志级别至少区分 `Debug`、`Information`、`Warning`、`Error`、`Critical`。
4. 禁止输出敏感信息：密码、令牌、证件号、银行卡完整信息。
5. 日志必须包含时间戳（ISO 8601）、服务名、实例标识。
6. 生产环境默认日志级别为 `Information`，`Debug` 级别仅在排查问题时临时开启。
7. 日志输出目标必须可配置（Console / 文件 / 日志收集 Agent），禁止硬编码。
8. 使用 `ILogger<T>` 注入，禁止直接 `new` 日志实例或使用静态日志方法。
9. 推荐使用高性能日志 API（`LoggerMessage.Define` 或 Source Generator `[LoggerMessage]`），热路径避免装箱和字符串插值。

### SHOULD
1. 日志采集推荐统一收集到日志平台（ELK / Loki / Seq / 云日志服务），支持集中检索。
...
5. 推荐使用 Serilog File Sink 配合 `rollingInterval` 和 `fileSizeLimitBytes` 实现，配置示例：
   ```csharp
...
3. 后台任务必须有吞吐量、失败数、重试次数指标。
4. 指标暴露格式推荐 Prometheus 兼容（`/metrics` 端点），使用 `prometheus-net` 或 OpenTelemetry Metrics Exporter。
5. 自定义业务指标（如订单量、支付成功率）必须使用统一指标库注册，禁止散写。

### 资源指标（MUST）
1. 以下运行时指标必须暴露：
   - .NET Runtime：GC 停顿时间、堆内存使用、线程池活跃/排队数。
   - 连接池：活跃连接数、空闲连接数、等待数。
   - 消息队列：消费延迟、积压量（微服务场景）。
...
1. 必须接入分布式链路追踪系统（推荐 OpenTelemetry .NET SDK）。
2. 追踪协议使用 **W3C Trace Context**（`traceparent` / `tracestate`），禁止自定义私有协议。
3. 跨服务调用必须透传 trace 上下文：
   - HTTP：通过 `traceparent` Header 传播（`HttpClient` 自动支持）。
   - gRPC：通过 metadata 传播。
   - 消息队列：通过消息 Header/属性传播。
4. 关键业务流程必须具备端到端可追踪能力（从前端请求到最终数据库写入）。
5. Activity/Span 命名规范：`{服务名}.{层级}.{操作}`（如 `OrderService.Service.CreateOrder`）。
6. Activity 必须记录关键属性：HTTP 方法/路径/状态码、gRPC 方法/状态码、数据库语句（脱敏）、错误信息。
7. 配置示例：
   ```csharp
